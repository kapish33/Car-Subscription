...
var commonValidations = {
  id: import_zod3.z.string().refine((data) => typeof data === "string", {
...
var UserSchema2 = import_zod4.z.object({
  _id: import_zod4.z.string().uuid(),
  firstName: import_zod4.z.string(),
  email: import_zod4.z.string().email(),
  lastName: import_zod4.z.string(),
  age: import_zod4.z.number(),
  password: import_zod4.z.string(),
  resetToken: import_zod4.z.string().optional(),
  resetTokenExpiry: import_zod4.z.number().optional()
  // createdAt: z.date(),
  // updatedAt: z.date(),
});
var TokensSchema = import_zod4.z.object({
  accessToken: import_zod4.z.string(),
  refreshToken: import_zod4.z.string()
});
var UserWithTokensSchema = import_zod4.z.object({
  user: UserSchema2,
  ...TokensSchema.shape
});
...
var LoginPayload = import_zod4.z.object({
  body: import_zod4.z.object({
    email: import_zod4.z.string().email(),
    password: import_zod4.z.string()
  })
});
var ResetPasswordPayload = import_zod4.z.object({
  body: import_zod4.z.object({
    email: import_zod4.z.string().email()
  })
});
var ResetNewPasswordPayload = import_zod4.z.object({
  body: import_zod4.z.object({
    token: import_zod4.z.string(),
    newPassword: import_zod4.z.string()
  })
});
var ResponseString = import_zod4.z.object({
  responseObject: import_zod4.z.string()
});
...
var requestCookieValidation = async (req, res, next) => {
  ...
  if (!accessToken && !refreshToken) {
    return res.status(import_http_status_codes4.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "No tokens provided" });
  }
  try {
    ...
    if (!refreshToken) {
      return res.status(import_http_status_codes4.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid access token and no refresh token provided" });
    }
    try {
      const { user } = import_jsonwebtoken2.default.verify(refreshToken, env.JWT_REFRESH_SECRET);
      ...
      return res.status(import_http_status_codes4.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid refresh token" });
    }
  }
};
...
userRegistry.register("User", UserSchema2);
var userRouter = (() => {
  ...
  userRegistry.registerPath({
    ...
    path: "/users",
    tags: ["User"],
    responses: createApiResponse(import_zod5.z.array(UserSchema2), "Success")
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(UserSchema2, "Success")
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(UserWithTokensSchema, "User created successfully", import_http_status_codes5.StatusCodes.CREATED)
  });
  router.post("/create", validateRequest(CreateUserSchema), async (req, res) => {
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(TokensSchema, "User logged in successfully", import_http_status_codes5.StatusCodes.OK)
  });
  router.post("/login", validateRequest(LoginPayload), async (req, res) => {
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password reset email sent", import_http_status_codes5.StatusCodes.OK)
  });
  router.patch("/reset/password", validateRequest(ResetPasswordPayload), async (req, res) => {
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password Updated successfully", import_http_status_codes5.StatusCodes.OK)
  });
  router.patch("/verify-reset-token", validateRequest(ResetNewPasswordPayload), async (req, res) => {
    const verifyResetData = req.body;
    const serviceResponse = await userService.setNewUserPassword(verifyResetData);
    handleServiceResponse(serviceResponse, res);
  });
  router.get("/validate/k", requestCookieValidation, async (_req, res) => {
    res.send({
      success: true
    });
  });
...
var unexpectedRequest = (_req, res) => {
  res.sendStatus(import_http_status_codes6.StatusCodes.NOT_FOUND);
};
...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= import_http_status_codes7.StatusCodes.INTERNAL_SERVER_ERROR) return "error" /* Error */;
  if (res.statusCode >= import_http_status_codes7.StatusCodes.BAD_REQUEST) return "warn" /* Warn */;
  if (res.statusCode >= import_http_status_codes7.StatusCodes.MULTIPLE_CHOICES) return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === import_http_status_codes7.StatusCodes.NOT_FOUND) return (0, import_http_status_codes7.getReasonPhrase)(import_http_status_codes7.StatusCodes.NOT_FOUND);
...
var connectDB = async () => {
  try {
    await import_mongoose2.default.connect(env.MONGO_DB_URL);
...
var userService = {
  findAll: async () => {
    try {
      ...
      if (!users) {
        return new ServiceResponse(1 /* Failed */, "No Users found", null, import_http_status_codes8.StatusCodes.NOT_FOUND);
      }
      return new ServiceResponse(0 /* Success */, "Users found", users, import_http_status_codes8.StatusCodes.OK);
    } catch (ex) {
      const errorMessage = `Error finding all users: ${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes8.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  },
  findById: async (id) => {
    try {
      ...
      if (!user) {
        return new ServiceResponse(1 /* Failed */, "User not found", null, import_http_status_codes8.StatusCodes.NOT_FOUND);
      }
      return new ServiceResponse(0 /* Success */, "User found", user, import_http_status_codes8.StatusCodes.OK);
    } catch (ex) {
  ...
  createUser: async (newUser) => {
    try {
      ...
      return new ServiceResponse(
        ...
        user,
        import_http_status_codes8.StatusCodes.CREATED
      );
    } catch (ex) {
      const errorMessage = `Error creating user: ${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes8.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  },
  loginUser: async (loginPayload) => {
    try {
      const user = await userRepository.loginUserAsync(loginPayload);
      if (!user) {
        return new ServiceResponse(1 /* Failed */, "Invalid email or password", null, import_http_status_codes8.StatusCodes.UNAUTHORIZED);
      }
      return new ServiceResponse(
        0 /* Success */,
        "User logged in successfully",
        user,
        import_http_status_codes8.StatusCodes.OK
      );
    } catch (ex) {
      const errorMessage = `Error logging in user: ${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes8.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  },
  resetUserPassword: async (resetPayload) => {
    try {
      ...
      const errorMessage = `Error sending password reset email: ${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes8.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  },
  setNewUserPassword: async (newPasswordPayload) => {
    try {
      const result = await userRepository.setNewUserPassAsync(newPasswordPayload);
      if (!result) {
        return new ServiceResponse(1 /* Failed */, "Invalid or expired token", null, import_http_status_codes8.StatusCodes.BAD_REQUEST);
      }
      return new ServiceResponse(
        0 /* Success */,
      ...
      const errorMessage = `Error updating password: ${ex.message}`;
      logger.error(errorMessage);
      return new ServiceResponse(1 /* Failed */, errorMessage, null, import_http_status_codes8.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
...
describe("userService", () => {
  ...
  describe("findAll", () => {
    it("return all users", async () => {
      ...
      expect(result.statusCode).toEqual(import_http_status_codes9.StatusCodes.OK);
    ...
    it("returns a not found error for no users found", async () => {
      ...
      const result = await userService.findAll();
      expect(result.statusCode).toEqual(import_http_status_codes9.StatusCodes.NOT_FOUND);
    ...
    it("handles errors for findAllAsync", async () => {
      ...
      const result = await userService.findAll();
      expect(result.statusCode).toEqual(import_http_status_codes9.StatusCodes.INTERNAL_SERVER_ERROR);
      expect(result.success).toBeFalsy();
