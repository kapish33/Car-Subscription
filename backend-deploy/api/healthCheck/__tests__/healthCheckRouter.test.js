...
function dist_default(len) {
  ...
  return function() {
    if (!str2 || num2 === 256) {
      str2 = "";
      num2 = (1 + len) / 2 | 0;
      while (num2--) str2 += HEX[256 * Math.random() | 0];
    ...
    (function() {
      ...
      function CookieJar() {
        ...
        if (this instanceof CookieJar) {
          ...
          this.setCookie = function setCookie2(cookie, request_domain, request_path) {
            var remove, i;
...
var UserSchema2 = new import_mongoose.Schema(
  {
...
var UserModel = import_mongoose.default.model("User", UserSchema2);
...
var userService = {
  findAll: async () => {
    try {
      ...
      return new ServiceResponse(0 /* Success */, "Users found", users, import_http_status_codes4.StatusCodes.OK);
    } catch (ex) {
  ...
  loginUser: async (loginPayload) => {
    try {
      ...
      if (!user) {
        return new ServiceResponse(1 /* Failed */, "Invalid email or password", null, import_http_status_codes4.StatusCodes.UNAUTHORIZED);
      }
      return new ServiceResponse(
        0 /* Success */,
  ...
  setNewUserPassword: async (newPasswordPayload) => {
    try {
      ...
      if (!result) {
        return new ServiceResponse(1 /* Failed */, "Invalid or expired token", null, import_http_status_codes4.StatusCodes.BAD_REQUEST);
      }
      return new ServiceResponse(
        0 /* Success */,
...
var requestCookieValidation = async (req, res, next) => {
  ...
  if (!accessToken && !refreshToken) {
    return res.status(import_http_status_codes5.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "No tokens provided" });
  }
  try {
    ...
    if (!refreshToken) {
      return res.status(import_http_status_codes5.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid access token and no refresh token provided" });
    }
    try {
      const { user } = import_jsonwebtoken2.default.verify(refreshToken, env.JWT_REFRESH_SECRET);
      ...
      return res.status(import_http_status_codes5.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid refresh token" });
    }
  }
};
...
var userRouter = (() => {
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(UserWithTokensSchema, "User created successfully", import_http_status_codes6.StatusCodes.CREATED)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(TokensSchema, "User logged in successfully", import_http_status_codes6.StatusCodes.OK)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password reset email sent", import_http_status_codes6.StatusCodes.OK)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password Updated successfully", import_http_status_codes6.StatusCodes.OK)
  });
...
describe("Health Check API endpoints", () => {
  it("GET / - success", async () => {
    ...
    expect(response.statusCode).toEqual(import_http_status_codes9.StatusCodes.OK);
