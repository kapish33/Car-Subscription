...
var userService = {
  findAll: async () => {
    try {
      ...
      return new ServiceResponse(0 /* Success */, "Users found", users, import_http_status_codes6.StatusCodes.OK);
    } catch (ex) {
  ...
  loginUser: async (loginPayload) => {
    try {
      ...
      if (!user) {
        return new ServiceResponse(1 /* Failed */, "Invalid email or password", null, import_http_status_codes6.StatusCodes.UNAUTHORIZED);
      }
      return new ServiceResponse(
        0 /* Success */,
  ...
  setNewUserPassword: async (newPasswordPayload) => {
    try {
      ...
      if (!result) {
        return new ServiceResponse(1 /* Failed */, "Invalid or expired token", null, import_http_status_codes6.StatusCodes.BAD_REQUEST);
      }
      return new ServiceResponse(
        0 /* Success */,
...
var requestCookieValidation = async (req, res, next) => {
  ...
  if (!accessToken && !refreshToken) {
    return res.status(import_http_status_codes7.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "No tokens provided" });
  }
  try {
    ...
    if (!refreshToken) {
      return res.status(import_http_status_codes7.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid access token and no refresh token provided" });
    }
    try {
      const { user } = import_jsonwebtoken2.default.verify(refreshToken, env.JWT_REFRESH_SECRET);
      ...
      return res.status(import_http_status_codes7.StatusCodes.UNAUTHORIZED).json({ valid: false, message: "Invalid refresh token" });
    }
  }
};
...
var userRouter = (() => {
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(UserWithTokensSchema, "User created successfully", import_http_status_codes8.StatusCodes.CREATED)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(TokensSchema, "User logged in successfully", import_http_status_codes8.StatusCodes.OK)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password reset email sent", import_http_status_codes8.StatusCodes.OK)
  });
  ...
  userRegistry.registerPath({
    ...
    responses: createApiResponse(ResponseString, "Password Updated successfully", import_http_status_codes8.StatusCodes.OK)
  });
