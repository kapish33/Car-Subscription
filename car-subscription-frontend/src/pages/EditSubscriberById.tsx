...
const fetchSubscription = async (
  ...
  try {
    ...
    // Transforming responseObject to match the form structure
    const transformedData: FormData = {
      user: responseObject.user._id,
      carType: responseObject.carType,
      planType: responseObject.planType,
      startDate: new Date(responseObject.startDate),
      schedule: responseObject.schedule.map((item: any) => ({
...
export default function Scheduler() {
  ...
  const {
    ...
    defaultValues: {
      schedule: [{ date: undefined, serviceType: "", timeSlot: "" }],
    },
  });
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const response = await axiosInstance.patch(`/subscription/${id}`, data);
      ...
      // redirect('/dashboard/all-schedule');
      navigate("/dashboard/all-schedule");
    } catch (error) {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                append({ date: undefined, serviceType: "", timeSlot: "" })
              }
            >
